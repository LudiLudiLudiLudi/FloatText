...
final class PanelState: ObservableObject {
    ...
    init() {
        ...
        // Background strength. Band [0.35, 1.0]: 0.35 = noticeably
        // transparent, 1.0 = fully opaque dark. Only the backing layer is
        // affected; text keeps full alpha. Default 0.75.
        let storedOpacity = d.object(forKey: K.opacity) as? Double ?? 0.75
        let clamped = max(0.35, min(1.0, storedOpacity))
